...
export default function Quotes() {
	...
	const quoteAPI = async () => {
		let arrayQuotes = [];
		try {
			const data = await axios.get("https://api.forismatic.com/api/1.0/?method=getQuote&lang=en&format=jsonp&jsonp=?");
		...
		try {
			setQuote(arrayQuotes.quoteText);
			setAuthor(arrayQuotes.quoteAuthor);
		}
		catch(err)
		{
			console.log(err);
